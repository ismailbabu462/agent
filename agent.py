...
class DesktopAgent:
    ...
    async def handle_message(self, websocket, message: str):
        ...
        try:
            ...
            if data.get('type') == 'execute_tool':
            ...
            elif data.get('type') == 'install_tool':
                # NEW: Tool installation from Tool Store
                tool_id = data.get('tool_id')
                tool_name = data.get('tool_name')
                installation_command = data.get('installation_command')
                session_token = data.get('session_token')
                
                if not tool_id or not installation_command:
                    await self.send_to_client(websocket, {
                        'type': 'installation_error',
                        'tool_id': tool_id,
                        'message': 'Missing tool_id or installation_command'
                    })
                    return
                
                # SECURITY: Check if tool is in whitelist
                if tool_id not in ALLOWED_TOOLS:
                    logger.warning(f"SECURITY: Unauthorized tool installation attempt: {tool_id}")
                    await self.send_to_client(websocket, {
                        'type': 'installation_error',
                        'tool_id': tool_id,
                        'message': f'Tool {tool_id} is not in approved whitelist'
                    })
                    return
                ...
                await self.send_to_client(websocket, {
                    ...
                    'tool_id': tool_id,
                    'tool_name': tool_name or ALLOWED_TOOLS[tool_id]['description'],
                    'message': f'Starting installation of {tool_id}...'
                })
                
                # Install the tool
                success = await self.install_tool(tool_id, websocket)
                ...
                if success:
                    await self.send_to_client(websocket, {
                        ...
                        'tool_name': tool_name or ALLOWED_TOOLS[tool_id]['description'],
                        'success': True,
                        'message': f'{tool_id} installed successfully!'
                    })
                else:
                    await self.send_to_client(websocket, {
                        'type': 'installation_error',
                        'tool_id': tool_id,
                        'success': False,
                        'message': f'Failed to install {tool_id}. Please check requirements.'
                    })
            
            elif data.get('type') == 'check_tool':
                # NEW: Check if tool is installed
                tool_id = data.get('tool_id')
                
                if tool_id not in ALLOWED_TOOLS:
                    await self.send_to_client(websocket, {
                        'type': 'tool_check_result',
                        'tool_id': tool_id,
                        'installed': False
                    })
                    return
                
                is_installed = self.is_tool_available(tool_id)
                await self.send_to_client(websocket, {
                    'type': 'tool_check_result',
                    'tool_id': tool_id,
                    'installed': is_installed,
                    'command': ALLOWED_TOOLS[tool_id]['command']
                })
